    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          ...
          <p className="mt-1 text-sm text-[#514349]">View your registered customer base and their lifetime value.</p>
        </div>
      </div>

      <div className="rounded-xl border border-[#D5C1C9]/30 bg-white">
        {isPending ? (
          <div className="p-8 text-center text-sm text-[#514349]">Loading customers...</div>
        ) : customers.length === 0 ? (
          <div className="rounded-xl p-16 text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-primary/10">
              <Users className="h-6 w-6 text-primary" />
            </div>
            <p className="text-sm text-[#514349]">No customers found.</p>
          </div>
        ) : (
                ...
                {customers.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className="transition-colors hover:bg-slate-50">
                      ...
                      <td className="px-5 py-4 text-[#514349]">
                        {c.email}
                      </td>
                      <td className="px-5 py-4 text-[#514349]">
                        {c.phone || '—'}
                      </td>
                      <td className="px-5 py-4 text-[#191C1E] font-medium">
                        {c.orderCount}
                      </td>
                      <td className="px-5 py-4 font-medium text-primary">
                        {formatPrice(c.totalSpentCents || 0, 'CAD')}
                      </td>
                      <td className="px-5 py-4 text-right text-xs text-[#514349]">
